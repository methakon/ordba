...
class Posts_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Post'; // the name of the record class (if it can't be determined)
	public $filters = array(); // filters to apply to when searching for items
	public $required = array('title','Slug','content'); // an array of required fields. If a key => val is provided, the key is name of the field and the value is the error message to display
	...
	public $serialized_fields = array('related_posts'); // fields that contain serialized data. This will automatically serialize before saving and unserialize data upon retrieving
	public $has_many = array(
             'related_posts' => array('model' => array( 'related' => 'Posts_model' ),'where' => array( )),
	...
	{
		parent::__construct('fuel_posts'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		$this->db->select('id, title,precedence,Slug,date_added,date_modified');  
...
class Post_model extends Base_module_record {
